...
marts.revenue_by_category_daily`,
      ...
      },
      {
        ...
        chipsLabel: 'Iceberg Capabilities', chips: ['ACID transactions', 'Hidden partitioning', 'Snapshot isolation', 'Engine-agnostic metadata'],
  ...
  "location": "abfss://lakehouse@cxosdata.dfs.core.windows.net/curated/events"
        ...
        integration: [
          ...
          'Any Iceberg-compatible engine (Spark, Trino) — can attach without migration',
        ],
        nfr: [
...
-- Existing files remain valid; new column reads as NULL for old rows`,
    ...
    items: [
      {
        ...
        hld: [
          ...
          { label: 'Foundation', name: 'Access Control (RBAC/ABAC)', detail: 'Azure AD roles + attribute-based policy evaluation', origin: true },
        ...
        technical: "RBAC assigns broad table-level permissions via Azure AD (Entra ID) group membership (e.g., 'data-engineering' can read raw and curated zones). ABAC layers finer-grained, attribute-based rules on top — e.g., 'customer-support agents can read profile data only for customers in their assigned region.' Both are evaluated by the same shared policy library described under Policies &amp; Entitlements, checked on every query before data is returned.",
        chipsLabel: 'Model', chips: ['RBAC — role-based (Azure AD groups)', 'ABAC — attribute-based (finer-grained rules)'],
  ...
  "constraints": { "column_mask": ["email", "phone"] }
        ...
        nfr: [
          ...
          'Security/Privacy: a foundational security control; access decisions are themselves logged for audit',
        ],
        example: "A support agent based in the EU tries to query a US customer's profile. ABAC's region-attribute rule denies the request even though their RBAC role would otherwise permit customer-profile access — a more precise control than role alone could provide.",
      },
      {
